...
def is_valid(numbers, result):
    ...
    if len(numbers) == 2:
        ...
        or_result = int(str(numbers[0]) + str(numbers[1]))

        mult_numbers = [mult_result] + numbers[2:]
        add_numbers = [add_result] + numbers[2:]
        or_numbers = [or_result] + numbers[2:]

        return is_valid(mult_numbers, result) or is_valid(add_numbers, result) or is_valid(or_numbers, result)
